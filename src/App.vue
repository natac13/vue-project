...
const includingVat = computed(() => {
  return (price.value * tax).toFixed(2)
})

const total = computed(() => {
  const t = price.value * items.value * tax
  return t.toFixed(2)
})

const vat = () => {
  return (price.value * tax - price.value).toFixed(2)
}
...
const ListShema = z.array(PokemonSchema)

async function getPokemon() {
  // const response = await fetch('https://pokeapi.co/api/v2/pokemon?limit=151')
  // const data = await response.json()

  // const result = ListShema.parse(data.results)
  const result = [
...
<template>
  ...
  <p>Including VAT: {{ includingVat }}</p>
  <p>Total: {{ total }}</p>
  <p>VAT: {{ vat() }}</p>

  <header class="bg-gray-800 text-white p-6">
    <h1 class="text-2xl">Pokémon</h1>
  </header>
  <ul class="container max-w-3xl mx-auto p-6 list-decimal space-y-4">
    <li v-for="p in pokemon" :key="p.name" class="bg-gray-200 rounded-md p-6">
      <div class="flex flex-col gap-1">
        <span>
          {{ p.name }}
        </span>
        <a :href="p.url" target="_blank" class="text-blue-500 underline">
          {{ p.url }}
        </a>
      </div>
    </li>
  ...
  <button @click="getPokemon">Fetch Pokemon</button>
</template>
